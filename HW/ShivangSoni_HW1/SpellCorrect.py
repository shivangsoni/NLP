...
class SpellCorrect:
  ...
  def correctSentence(self, sentence):
    ...
    for i in range(1, len(sentence) - 1):
      ...
      Prob = self.editModel.editProbabilities(keeptrack)
      for newWord, candidateModify in Prob:
        if newWord == keeptrack:
          continue
        sentence[i] = newWord
        score = self.languageModel.score(sentence)
        netScore = score + candidateModify
        if netScore >= bestScore:
          bestIndex = i
          bestScore = netScore
          bestWord = newWord
        sentence[i] = keeptrack 
    bestSentence[bestIndex] = bestWord
    return bestSentence
