...
class EditModel(object):
  ...
  def deleteEdits(self, word):
    ...
    for i in xrange(1, len(word)):
      ...
      #print Edit(correction, corruptLetters, correctLetters)
  ...
  def insertEdits(self, word):
    ...
    for i in xrange(0, len(word)):
       ...
       for j in xrange(0, len(EditModel.ALPHABET)):
          correctLetters = word[i]+EditModel.ALPHABET[j]
  ...
  def transposeEdits(self, word):
   ...
   for i in range(0,len(word)-1):
      corruptLetters = word[i] + word[i+1]
      correctLetters = word[i+1] + word[i]
    ...
    for i in xrange(1, len(word)):
       ...
       for j in xrange(0, len(EditModel.ALPHABET)):
          if (word[i] != EditModel.ALPHABET[j]): 
            ...
            correction = "%s%s%s" % (word[1:i],EditModel.ALPHABET[j],word[i+1:])
            ret.append(Edit(correction, corruptLetters, correctLetters))
    return ret  
