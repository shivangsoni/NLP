...
class NaiveBayes:
    ...
    def classify(self, words):
        ...
        numPOS = math.log((self.totalPositiveWords)/(self.totalPositiveWords + self.totalNegativeWords)) #POS NUM
        numNEG = math.log((self.totalNegativeWords)/(self.totalPositiveWords + self.totalNegativeWords)) #NEG NUM
        
        for word in words:
          numPOS += math.log((self.posWordList.get(word,0)+1.8)/(1.8*self.totalPositiveWords + len(self.netWordList)))
          numNEG += math.log((self.negWordList.get(word,0)+1.8)/(1.8*self.totalNegativeWords + len(self.netWordList)))
    ...
    def not_never(self, words):
       neg_words = re.compile("^not$|never|[A-Z][a-b]n't$");
       print(neg_words)
       cont = False
       wordList=[]
       for word in words:
