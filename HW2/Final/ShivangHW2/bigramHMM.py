    ...
    for line in inputfile.read().splitlines():
        ...
        for k, word in enumerate(line):
            ...
            for w,u,v in itertools.product(tags, tags, tags): #python nested for loop
                ...
                if (w, u, v) in trans and (u, word) in emit and (k - 1, v) in pi:
                    p = pi[(k - 1, u, v)] + trans[(w,u,v)] + emit[(v, word)]
        ...
        if foundgoal:
            ...
            y = []
            for i,k in xrange(len(line)-2, 0, -1): #counting from the last word
                # bp[(i, tag)] gives you the tag for word[i - 1].
                # we use that and traces through the tags in the sentence.
                y.append(bp[(k+2, tags[i+1], tags[i])])
                tag = bp[(k+2, tags[i+1], tags[i])]
